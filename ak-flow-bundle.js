(function(){
  'use strict';
  // AK-FLOW v1.8 - Staffbase Custom Widget Bundle
 // Fix: Syntax error in mailto body + scroll fix + compact layout
          ...
          if(bubble){
                  bubble.querySelectorAll('.ak-chat-tile').forEach(function(tile){
                            tile.onclick=function(){
                                        ...
                                        var sel=Array.from(bubble.querySelectorAll('.ak-chat-tile.sel')).map(function(t){return t.dataset.art;});
                                        akState.art=sel;
                            };
                  });
          }
 ...
 function step2(area,grid){
    ...
    setTimeout(function(){
          actionBar(area,grid,[
            {label:'\u2190 Zur\u00fcck',cls:'secondary',fn:function(){
    ...
    setTimeout(function(){
          actionBar(area,grid,[
            ...
            {label:'\u2705 Jetzt bestellen',cls:'green',fn:function(){
                      ...
                      sendEmail(orderNum);
                      botMsg(area,'<div class="ak-success-box"><div style="font-size:22px;">\u2705</div><div class="ak-order-num">'+orderNum+'</div><div style="font-size:11px;color:#555;">Deine Bestellung wurde erfolgreich eingereicht und an deinen Genehmiger weitergeleitet.</div></div>',grid,400);
                      setTimeout(function(){
                                  actionBar(area,grid,[
                                    {label:'\ud83d\udd04 Neue Bestellung',fn:function(){
                                                    while(area.lastChild)area.removeChild(area.lastChild);
                step1(area,grid);
                                    }}
                                              ]);
 ...
 function watchAndAttach(){
     ...
     akObs=new MutationObserver(function(){
           attach();
     });
